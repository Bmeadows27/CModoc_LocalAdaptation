...
project_directory <- "/lustre/scratch/brameado/Blochmannia_Local_Adaptation"
directory_name <- "11_genotype_scripts"
reference_genome_location <- "/lustre/scratch/brameado/Blochmannia_Local_Adaptation/2_modoc.fasta"
cluster <- "quanah"
output_name <- "blochmannia_genotype"
popmap <- "blochmannia_popmap.txt"
individuals <- read.table(popmap)
faidx <- read.table("2_modoc.fasta.fai", sep="\t", stringsAsFactors=F)
...
gatk_command <- paste0('singularity exec $SINGULARITY_CACHEDIR/', name_of_gatk_singularity_image, ' gatk --java-options "-Xmx', total_mem1, 'g" HaplotypeCaller -R ', reference_genome_location, " -I ", a_name, " --sample-ploidy 1", " -ERC GVCF -O ", project_directory, "/02_vcf/", "${name_array}", "._${ind_array}_.g.vcf", " --QUIET --intervals ", "${chr_array}")
