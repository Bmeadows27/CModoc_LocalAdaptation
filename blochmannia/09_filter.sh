...
vcftools --vcf ${workdir}/03_vcf/${input_array}.g.vcf  --max-missing 1.0 --minQ 20 --minGQ 20 --minDP 10 --min-alleles 2 --max-alleles 2 --maf 0.05 --remove-indels --recode --recode-INFO-all --out ${workdir}/04_filter_vcf_admixture/${input_array}

#run vcftools with SNP output for GEA tests
vcftools --vcf ${workdir}/03_vcf/${input_array}.g.vcf  --max-missing 0.8 --minQ 20 --minGQ 20 --minDP 10 --min-alleles 2 --max-alleles 2 --maf 0.05 --remove-indels --recode --recode-INFO-all --out ${workdir}/06_filter_vcf_GEA/${input_array}
