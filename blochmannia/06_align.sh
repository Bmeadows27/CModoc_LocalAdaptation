...
# run bbduk
/lustre/work/jmanthey/bbmap/bbduk.sh in1=${workdir}/01_blochDNA/${basename_array}_R1.fastq.gz in2=${workdir}/01_blochDNA/${basename_array}_R2.fastq.gz out1=${workdir}/02_bloch_cleaned/${basename_array}_R1.fastq.gz out2=${workdir}/02_bloch_cleaned/${basename_array}_R2.fastq.gz minlen=50 ftl=10 qtrim=rl trimq=10 ktrim=r k=25 mink=7 ref=/lustre/work/jmanthey/bbmap/resources/adapters.fa hdist=1 tbo tpe

# run bwa mem
bwa mem -t 12 ${refgenome} ${workdir}/02_bloch_cleaned/${basename_array}_R1.fastq.gz ${workdir}/02_bloch_cleaned/${basename_array}_R2.fastq.gz > ${workdir}/01_bam_files/${basename_array}.sam
